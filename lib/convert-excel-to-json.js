...
const excelToJson = (function() {
    ...
    // Since columns are characters, and a column can contain more than one character,
    // this function will convert the characters into a number to compare column ranges.
    const toCharValue = (column) => [...column].reduce((sum, ch) => sum + ch.charCodeAt(0), 0);

    function columnIsInRange(strictRangeColumns, column) {
        const columnValue = toCharValue(column);
        return columnValue < strictRangeColumns.from || columnValue > strictRangeColumns.to;
    }
    ...
    const parseSheet = (sheetData, workbook) => {
        ...
        if (range) {
            strictRangeColumns = {
                from: toCharValue(getCellColumn(getRangeBegin(range))),
                to: toCharValue(getCellColumn(getRangeEnd(range)))
            };
        ...
        for (let cell in sheet) {
            ...
            if ((strictRangeColumns && strictRangeRows) && (columnIsInRange(strictRangeColumns, column) || row < strictRangeRows.from || row > strictRangeRows.to)) {
                continue;
            }
